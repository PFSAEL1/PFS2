    ...
    <section className="py-16 section-white">
      <div className="container">
        <div style={{ textAlign: "center", maxWidth: "780px", margin: "0 auto 2.5rem" }}>
          ...
          <h2 data-animation="slideLeft" className="section-heading">{title}</h2>
          <div className="red-divider" style={{ margin: "0.75rem auto 1rem" }} />
          <p data-animation="slideLeft"className="section-body" style={{ margin: 0 }}>{intro}</p>
        </div>

        <div data-animation="fadeIn" className="grid lg:grid-cols-2 gap-6" style={{ maxWidth: "1080px", margin: "0 auto" }}>
          {cards.map((card) => (
            <article key={card.heading} className="card-hover" style={{ background: "#f6f6f4", border: "1px solid #e7e7e2", borderRadius: "2px", overflow: "hidden" }}>
              <div className="card-image" style={{ background: "#fff", aspectRatio: "4 / 3", display: "flex", alignItems: "center", justifyContent: "center", padding: "clamp(1rem, 2vw, 1.5rem)" }}>
              ...
              <div style={{ padding: "1.45rem" }}>
                ...
                <h3 data-animation="slideLeft" style={{ fontFamily: "'Chakra Petch', 'Barlow Condensed', sans-serif", fontSize: "1.08rem", fontWeight: 800, color: "#1C1C1E", letterSpacing: "0.015em", textTransform: "uppercase", margin: "0.5rem 0 0.65rem" }}>{card.heading}</h3>
                <p data-animation="slideRight"  style={{ fontFamily: "'Archivo Narrow', 'Inter', sans-serif", fontSize: "0.95rem", color: "#555", lineHeight: 1.68, margin: 0 }}>{card.copy}</p>
              </div>
            </article>
          ))}
        </div>
